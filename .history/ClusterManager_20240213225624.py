...
##########* create
    def create_containers(self, num_containers=8, name_list=None, volume=None):
        try:
            for i in range(num_containers):
                hostname = f"{self.cluster_name}-container{i+1}"
                container = self._create_container(hostname = hostname,name = name_list[i] if name_list else None, volume = volumes)
    ...
    def _create_container(self, hostname,name=None,volumes=volumes):
        # 将共享数据卷挂载到容器内的指定路径
...
####### processing data parallel
    def distribute_and_process_data_parallel(self,parallel_num, volume_data_path,container_idorname_list=None):
        try:
            if (parallel_num > len(self.containers)):
                ...
                return
            data = pickle.load(volume_data_path, 'rb')
            ...
            if(container_idorname_list):
                ...
                container_list = [self.client.containers.get(container_idorname) for container_idorname in self.containers[:parallel_num]]

            # 使用 ThreadPoolExecutor 进行并行执行
            with concurrent.futures.ThreadPoolExecutor() as executor:
                # 将每个容器的任务提交到执行器
                futures = [executor.submit(self._process_data_in_container, container, chunk_index)
    ...
    def _process_data_in_container(self, container, chunk_index):
        try:
