...
####### processing data parallel
    def distribute_and_process_data_parallel(self,parallel_num, volume_data_path, process_file_path,container_idorname_list=None):
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                # 将每个容器的任务提交到执行器
                futures = [executor.submit(self._process_data_in_container, container, chunk_index, volumneprocess_file_path)
    ...
    def _process_data_in_container(self, container, chunk_index, process_file_path):
        try:
            ...
            # 在容器中执行处理数据的命令
            self._execute_command_in_container(container_id, f"python {process_file_path} {chunk_index[0]} {chunk_index[1]}")
