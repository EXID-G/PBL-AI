...
##########* create
    ...
    def _create_container(self, hostname, name=None, volumes=None):
        container = self.client.containers.run('jupyter/scipy-notebook', command='sh',detach=True,stdin_open=True,tty=True, hostname=hostname, name=name, volumes = volumes)
...
##########* run
    ...
    def _execute_command_in_container(self, container_id, command):
        ...
        print("Command is being executed in container (id  ={container_id}).")
        exec_id = container.exec_run(command, stream = True)
        output_lines = []
        for line in exec_id:
        ...
        exit_code = container.exec_inspect(exec_id.id)['ExitCode']
        if exit_code == 0:
