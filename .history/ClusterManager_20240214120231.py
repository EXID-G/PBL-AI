...
##########* log 
    def log_status(self):
        for container in self.containers:
            ...
            # 设置下一个定时器
            threading.Timer(60.0, self.log_status).start()
...
##########* run
    ...
    def _execute_command_in_container(self, container_id, command):
        ...
        # Check container status, if not running, try to start container
        ...
        # get and print the output of command by stream
        print(f"\nCommand is being executed in container (id = {container_id}):")
        ...
        # check the exit code of command, since exit_code is always None, and online solution is not that useful. Besides. some people said 'it cannot return stream and exit code at the same time' in https://github.com/docker/docker-py/issues/1381. So, I just give up to get the exit code.
        ...
        logging.info(json.dumps(log_data))
         ...
         exec_id = container.exec_run(command) if exec_id.exit_code == 0: print(f"\nCommand executed successfully in Container (id ={container_id}):\n{exec_id.output.decode()}") else: print(f"\nCommand failed in Container (id ={container_id}):\n{exec_id.output.decode()}")
