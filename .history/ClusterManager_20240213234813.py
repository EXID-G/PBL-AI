...
####### processing data parallel
    def distribute_and_process_data_parallel(self,parallel_num, volume_data_path, cmd,container_idorname_list=None):
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                # 将每个容器的任务提交到执行器
                futures = [executor.submit(self._process_data_in_container, container, chunk_index, cmd)
    ...
    def _process_data_in_container(self, container, chunk_index, cmd):
        try:
            container_id = container.id

            # 在容器中执行处理数据的命令
            self._execute_command_in_container(container_id, command=cmd)
