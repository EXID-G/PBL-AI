...
####### processing data parallelly
    def distribute_and_process_data_parallel(self, data):
        try:
            num_containers = len(self.containers)
            chunk_size = len(data) // num_containers

            # 创建要分配给每个容器的数据块列表
            data_chunks = [data[i * chunk_size: (i + 1) * chunk_size] for i in range(num_containers)]
