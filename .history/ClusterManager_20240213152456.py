...
##########* create
    def create_containers(self, num_containers=8):
        choice = input("if you want to create containers with designated name (y/n): ")
        if choice == 'y':
            self.cluster_name = input("Enter the name list of the cluster (Separate with one space): ")
        try:
