    ...
    def distribute_and_process_data_parallel(self,parallel_num, volume_data_path, process_file_path,container_idorname_list=None):
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                # 将每个容器的任务提交到执行器
                futures = [executor.submit(self._process_data_in_container, container, chunk_index, process_file_path)
            ...
            print(f"Error distributing and process data parallel: {e}")

    def _process_data_in_container(self, container, chunk_index, process_file_path):
        try:
            container_id = container.id

            # 在容器中执行处理数据的命令
            self._execute_command_in_container(container_id, f"python ass2_volume/ass2_process_data.py {chunk_index}")

        except docker.errors.APIError as e:
            print(f"Error process data in container {container_id}: {e}")
